...
本專案使用純 HTML、CSS、JavaScript 實作，具備響應式設計、流暢動畫、多種 AI 難度，並支援 PWA（漸進式網頁應用），可在手機、平板、電腦上順暢遊玩。
...
- ** 五級 AI 難度**
  ...
  - **困難**：避免明顯不利的防守
  - **專家**：綜合局勢評估與開局庫
  - **特別**：使用 Minimax 搜尋演算法（深度 6 + Alpha-Beta 剪枝）
...
2. 點擊瀏覽器的「安裝應用程式」或「加到主畫面」提示
3. 遊戲將以獨立視窗執行，無需瀏覽器介面

---

##  遊戲規則（簡要）

1. **目標**：遊戲結束時，己方計分洞（右側大洞）內的棋子比對手多。
...
### 修改難度
在 `expertConditionalAI()` 與 `getSpecialAIMove()` 中調整評分權重或搜尋深度。
